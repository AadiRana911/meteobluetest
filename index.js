...
//   apiKey +
//   `&maps=${maps
//     .filter((map) => map.value !== "noaaMap") // Exclude NOAA map
//     .map((map) => map.value)
//     .join(
//       ","
//     )}&temperatureUnit=${temperatureUnit}&velocityUnit=${encodeURIComponent(
//     speedUnit
//   )}&lengthUnit=${lengthUnit}&overlays=pressure2mOverlay,graticuleOverlay`;
...
meteoblueMapboxMap.on("click", (e) => {
  ...
  const bounds = meteoblueMapboxMap.getBounds();
  const latRange = bounds.getNorth() - bounds.getSouth();
  const offsetLat = lat - latRange * 0.3; // Subtract to show point at top center

  meteoblueMapboxMap.easeTo({
